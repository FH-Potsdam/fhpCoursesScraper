...
function cleanDirtyObjectValue(obj){
	const reduceValidObject = (accumulator, key) => {
		if (obj[key] === null || obj[key] === NaN || obj[key] === undefined ) return accumulator;
...
class RawDataPrepare {
	...
	processStudyCode ( value ) {
		...
		if ( matches !== null && matches.length === 6 ) {
			...
			result.type = matches[ 4 ].toUpperCase();
		}
		return result;
	}
	...
	transformDataForDB( courses ) {

		return courses.map((course) => {

			var formattedCourse = {
				study: course.study,
				beginYear: this.softParseInt( course.beginYear ),
				summerSemester: course.summerSemester,
				studyCategoryId: this.softParseInt( course.studyCode.second ),
				studySubject: course.title.subject,
				studyField: course.studyCode.type,
				studyCode: course.studyCode.fullTitle,
				courseCapacity: this.softParseInt( course.description.maxAttendance ),
				title: course.title.title,
				description: course.description.description,
				//teachers: course.teachers,
				//locations: course.location,
				incomWorkspaceUrl: course.description.workspaceUrl,
				credits: course.credits
			}


			return cleanDirtyObjectValue(formattedCourse);
		});
